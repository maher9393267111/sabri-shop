...
const UpdateGalleryMain = ({ subcat }) => {
  ...
  const onFinish = async (values) => {
    ...
    // if file select from upload icon delete old image and add new image from file
      ...
      values.image = await uploadImages(file, true, "gallery");
      message.success("new image updated");
      await updateDoc(doc(db, "gallery", initialValues?.id), values);
...
export default UpdateGalleryMain;
